...
from .random_tools import (multinoulli, multinoulli_dict, random_randrange, random_choices,
...
def get_weight(network, config, dist_km, distance_histogram):
...
def assign_workplaces(network, config, activity_manager, work_activity_type, occupancy_houses,
    ...
    for house in tqdm(occupancy_houses):
        ...
        for location in work_locations_sample:
            ...
            dist_km = dist_m/1000
            weight = get_weight(network, config, dist_km, work_hist_dict['Luxembourg'])
    ...
    for border_country in occupancy_border_countries:
        for agent in tqdm(occupancy_border_countries[border_country]):
            ...
            for location in work_locations_sample:
                ...
                dist_km = dist_m/1000
                weight = get_weight(network, config, dist_km, work_hist_dict[border_country.typ])
...
def assign_locations_by_distance(network, config, activity_manager, activity_type,
    ...
    for house in tqdm(occupancy_houses):
        ...
        for location in visit_locations_sample:
            dist_m = euclidean_distance(house.coord, location.coord)
            dist_km = dist_m/1000
            weights_for_house[location] = get_weight(network, config, dist_km, hist_dict)
        for agent in occupancy_houses[house]:
...
def assign_cars(network, activity_manager, home_activity_type, car_activity_type, car_location_type,
    ...
    outdrs = network.locations_for_types(activity_manager.get_location_types(home_activity_type))
